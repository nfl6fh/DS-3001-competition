---
title: "Final_Project_Country_Random_Forest"
...
```{r}
ggplot(final_rap, aes(x=energy, y=popularity)) + geom_point(alpha=.1) + geom_smooth(method="lm", formula=y~x, se=T) + labs(title = "Song Popularity by 'Energy'") 

ggplot(final_rap, aes(x=loudness, y=popularity)) + geom_point(alpha=.1) + geom_smooth(method="lm", formula=y~x, se=T) + labs(title = "Song Popularity by 'Loudness'")
