---
title: "Final_Project_Rap_Random_Forest"
...
Visualization of key metrics for song popularity
```{r}
ggplot(final_rap, aes(x=speechiness, y=popularity)) + geom_point(alpha=.1) + geom_smooth(method="lm", formula=y~x, se=T) + labs(title = "Song Popularity by 'Speechiness'") 

ggplot(final_rap, aes(x=danceability, y=popularity)) + geom_point(alpha=.1) + geom_smooth(method="lm", formula=y~x, se=T) + labs(title = "Song Popularity by 'Danceability'")
